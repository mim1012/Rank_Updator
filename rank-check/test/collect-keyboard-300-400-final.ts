#!/usr/bin/env npx tsx
/**
 * "키보드" 키워드 300~400위권 상품 100개 수집 (성공 방식)
 *
 * ✅ 성공 패턴 사용:
 * 1. 네이버 메인 → 검색 → 쇼핑탭 클릭 (사람처럼)
 * 2. enterShoppingTab + goToPageAndGetAPIData
 * 3. humanType으로 자연스러운 타이핑
 */
...
async function collectProducts(page: any): Promise<ProductData[]> {
  ...
  // 3-1. 1페이지 진입 (DOM 방식으로 초기화)
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`📄 1페이지 초기화 중...`);
  await hydrateCurrentPage(page);
  await delay(2000);
  console.log(`   ✅ 1페이지 준비 완료`);

  // 3-2. 중간 페이지 자연스럽게 거쳐가기 (2~7페이지, 수집 안 함)
  if (startPage > 2) {
    console.log(`\n🚀 중간 페이지 거쳐가기 (2~${startPage - 1}페이지)...`);
    for (let skipPage = 2; skipPage < startPage; skipPage++) {
      console.log(`   📄 ${skipPage}페이지 통과 중...`);
      await goToPageAndGetAPIData(page, skipPage);
      ...
      // 충분한 딜레이 (3~5초)
      const quickDelay = 3000 + Math.random() * 2000;
      console.log(`   ⏳ ${(quickDelay / 1000).toFixed(1)}초 대기...`);
      await delay(quickDelay);
    }
    console.log(`   ✅ ${startPage - 1}페이지까지 통과 완료\n`);
  }

  // 3-3. 실제 수집 페이지 순회 (8~10페이지)
  for (let currentPage = startPage; currentPage <= endPage; currentPage++) {
    ...
    // 4. 수집한 상품을 전체 리스트에 추가 (300-400 범위만)
...
async function main() {
  console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`🎯 키보드 300-400위 상품 100개 수집 (성공 방식)`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  ...
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
  console.log(`📝 진입 방식: 네이버 메인 → 검색 → 쇼핑탭 (사람처럼)\n`);

  // 1. 브라우저 실행
  console.log('🚀 브라우저 실행 중...');

  const userDataDir = path.join(os.tmpdir(), `collect-keyboard-final-${Date.now()}`);
